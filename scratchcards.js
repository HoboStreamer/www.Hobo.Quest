...
var ScratchCards = {
    ...
    config: {
        actionURL: 'scratchcards_action.php',
        balanceNameSingular: 'credit',
        balanceNamePlural: 'credits',
        scratchLineWidth: 15,
        cellCompletePercentage: 0.45,
...
var ScratchCard = function($elScratchCard) {
    ...
    this.gameType = gameSettings.id;
    this.gameId = null;
    this.started = (gameState != null);
    this.cells = [];
    this.numCellsToOpen = parseInt(gameSettings.cells_to_open, 10);
    this.elScratchCard = $elScratchCard[0];

    this.$elDialogs = $elScratchCard.find(".scratch_card_dialog");
    this.elDialogOverlay = $elScratchCard.find(".scratch_card_dialog_overlay")[0];
    this.elStartGameDialog = $elScratchCard.find(".scratch_card_start_game_dialog")[0];
    this.elEndGameNoPrizeDialog = $elScratchCard.find(".scratch_card_end_game_no_prize_dialog")[0];
    this.elEndGameWithPrizeDialog = $elScratchCard.find(".scratch_card_end_game_with_prize_dialog")[0];
    this.elNoBalanceDialog = $elScratchCard.find(".scratch_card_no_balance_dialog")[0];
    this.elErrorLoggedOutDialog = $elScratchCard.find(".scratch_card_logged_out_dialog")[0];
    this.elErrorServerErrorDialog = $elScratchCard.find(".scratch_card_server_error_dialog")[0];
    this.elErrorFailedRequestDialog = $elScratchCard.find(".scratch_card_failed_request_dialog")[0];
    ...
    // $elScratchCard.find(".scratch_card_dialog_button_collect_prize").click(function(){ window.location.href = $(this).data("url"); });
...
ScratchCard.prototype.checkBalance = function(bet) {
    if (ScratchCards.balance < bet && bet > 0) {
...
ScratchCard.prototype.start = function(bet) {
    ...
    var params = { action: "start", game_type: this.gameType, bet: bet };
    this.makeRequest(ScratchCards.config.actionURL, params, function(data) {
        _this.started = true;
...
ScratchCard.prototype.generatePrizeText = function(prize, payout) {
    if (prize.prize_name) {
        ...
        return "You won " + this.formatPayout(payout);
    }
};
...
ScratchCard.prototype.makeRequest = function(url, params, fnSuccess) {
	...
	params.my_post_key = my_post_key;
	params.game_order_id = game_order_id;
